...
if __name__ == '__main__':
    ...
    with accelerator.main_process_first():
        if 'wikitext' in args.task_name:
            ...
            def tokenize_function(examples):
                return tokenizer(examples[text_column_name], max_length=5000, truncation=True)
    ...
    if args.sliding_window:
        ...
        def collate_fn(batch):
            ...
            if input_ids.shape[1] != block_size: # batch * input_size
                labels_mask = torch.ones_like(input_ids, dtype=bool)
                # Newly add
                # labels_mask[:, :-block_size] = False
                labels_mask[:, :-(input_ids.shape[1] % block_size) if input_ids.shape[1] % block_size !=0 else -block_size] = False
                collated['labels_mask'] = labels_mask
    ...
    if args.num_mem_tokens is not None:
        ...
        #------------------------------Load fined tuning model on segment 1 tokens 100-------------------------------------------------------
        path = '/home/ubuntu/cog/baseline/wikitext2/pytorch_model.bin'
        model.load_state_dict(torch.load(path))

        # Get update module # Newly add
        ...
        compress_module.wte.requires_grad_(False)
    ...
    # print(args.batch_size * args.gradient_accumulation_steps)
    # exit()
