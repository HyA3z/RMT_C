...
accelerate launch --config_file $ACCEL_CONFIG --main_process_port 29031 run_finetuning_babilong_rmt_long_c.py \
        --task_dataset $TASK_DATASET \
        --noise_dataset $NOISE_DATASET \
        --babi_path /root/recurrent-memory-transformer-babilong/data/tasks_1-20_v1-2/en-10k \
        --model_path /root/autodl-tmp/exp_record/babilong/rmt_c_4/2_fix/${TASK_DATASET}/$MODEL_NAME/${SCHEDULER}_adamw_wd1e-03_${MAX_N_SEGMENTS}x${SEGMENT_SIZE}_mem${MEMORY_SIZE}_bs${TBS}_bptt-${K2}_from_cpt_${SRC_N_SEGMENTS}-${MAX_N_SEGMENTS}/run_$N \
        ...
        --data_n_workers 2 \
        --log_interval $(($ITERS/100)) --valid_interval $(($ITERS/400)) \
